...
for gene_no = 1:trainingset_gene_count 
    ...
    if ((prom_pos > 0) & (prom_len - ppm_length - prom_pos > 0)) then
            ...
            promotor = prom_seq((prom_pos + 1): ppm_length + 1);
...
entropy_thresh = 0.045; // by observing the plot
...
for stat_align_thresh = -1:-1:-5
    ...
    for gene_index = testset_antisen_start_loc:1:selected_antisen_gene_count
        ...
        stat_align_score = stat_align(prom_seq, flipped_PPMatrix);
        reduced_stat_align_score = stat_align_entropy(prom_seq, flipped_PPMatrix, entropy_thresh);
